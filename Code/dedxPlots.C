...
/*
   TH1F* dedx1 = new TH1F("dedx1","dedx1", 100, 0., 2.);
   TH1F* dedx2 = new TH1F("dedx2","dedx2", 100, 0., 2.);
   TH1F* dedx3 = new TH1F("dedx3","dedx3", 100, 0., 2.);
   TH1F* dedx4 = new TH1F("dedx4","dedx4", 100, 0., 2.);
*/
   const int nbins = 80;
   double lowedge = -8.0;
   double highedge = 8.0;
   ...
   for (Long64_t jentry=0; jentry<100000;jentry++) {
      ...
      nLooped += 1;
      double_t mydedx = MyLayers();
      if(fabs( (mydedx - dedxnosat) ) > 1.e-6){
        nBadLay += 1;
	/*cout << "From Layer  data:  new " << mydedx
	     << " old " << dedxnosat << endl;*/
      }
      // Check if layer-level *FULL* can be reproduced
      double_t mydedx2 = MyLayersFull(0);
   ...
   cout << "From Raw    data:  " << nBadRaw
        << " bad out of " << nLooped << endl;
   cout << "From Layer  data:  " << nBadLay
        << " bad out of " << nLooped << endl;
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      nb = fChain->GetEntry(jentry);   nbytes += nb;
      // if (Cut(ientry) < 0) continue;
      //Set second set of dedx values here (currently set to layer recalculation)!!
      double_t mydedx = MyLayersFull(0);
      for( int i = 0; i < nbins; ++i ){
...
//Plotting Mean vs Momentum and resolution plots!
   ...
   gPad->SetGrid();
   ratio->Draw();
   p2 -> SaveAs("Trunc/MeanResPlot10.pdf");
