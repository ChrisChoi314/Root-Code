...
// Mon Jun  6 20:38:22 2022 by ROOT version 5.34/30
...
public :
   ...
   virtual Double_t     MyLayers(Double_t lbound, Double_t ubound );
...
#ifdef ResFit_cxx
ResFit::ResFit(TTree *tree) : fChain(0)
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   /*if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject(
         "/mnt/data/radb_sproc_lowadc.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("/mnt/data/radb_sproc_lowadc.root");
      }
   ...
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject(
         "/mnt/data/widget.33.e24.electron.radee.root");
      if (!f || !f->IsOpen()) {
...
Int_t ResFit::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t ResFit::LoadTree(Long64_t entry)
{
...
void ResFit::Show(Long64_t entry)
{
...
Int_t ResFit::Cut(Long64_t entry)
{
...
#endif // #ifdef ResFit_cxx

Double_t ResFit::MyLayers(Double_t lbound, Double_t ubound)
{
...
// ****************************************************
Double_t  ResFit::MyLayersFull(Int_t imode,Double_t lbound, Double_t ubound)
{
  vector<double> dedxlist;
  float mymean = 0.;
  int   mylNHits = 0;
...
// *** Truncation Tool ***
// ***********************
Double_t ResFit::MyTrunc(vector<double> dedxlist, Double_t lbound, Double_t ubound)
{
...
Double_t  ResFit::MyHits(Int_t imode,Double_t lbound, Double_t ubound)
{
  ...
  for (int idx=0; idx<hNHits; ++idx){
    ...
    if( hLayer[idx] >= lastlayer ){
      ...
      corr = scale*coscor*rungain*hWireGain[idx]*hPath[idx]*hOnedcor[idx]*
  ...
  double myhdedx = 0;
  myhdedx = MyTrunc(dedxlist,lbound, ubound);
