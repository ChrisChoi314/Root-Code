...
public :
   ...
   virtual Double_t     MyHits(Int_t imode,Double_t lbound,Double_t ubound,Double_t lenCut);
};
...
// *** Truncation Tool ***
// ***********************
Double_t Truncation::MyTrunc(vector<double> dedxlist, Double_t lbound, Double_t ubound)
{
...
Double_t  Truncation::MyHits(Int_t imode,Double_t lbound, Double_t ubound, Double_t lenCut)
{
  ...
  float myhdedx0, myhdedx1;

  float corr0,corr1;
  vector<double> dedxlist0,dedxlist1;
  ...
  for (int idx=0; idx<hNHits; ++idx){
    ...
    if( hLayer[idx] >= lastlayer ){
      ...
      corr0 = scale*coscor*rungain*hWireGain[idx]*hPath[idx]*hOnedcor[idx]*hTwodcor[idx]/sqrt(1-costh*costh);
      corr1 = scale*coscor*rungain*hWireGain[idx]*hPath[idx]*hOnedcor[idx]*hTwodcor[idx]/sqrt(1-costh*costh);

      // Method 0
      if( imode==0 && corr0!=0 && hPath[idx]>0.2){
      ...
      // Alternate method 1 logic ...
      // if( imode==1 && corr1!=0 &&
      //   ( (hLayer[idx]<8&&hPath>0.2) || (hLayer[idx]>7&&hPath>0.4)) ){
    }

  }
  myhdedx0 = MyTrunc(dedxlist0,lbound,ubound);
  myhdedx1 = MyTrunc(dedxlist1,lbound,ubound);

  double_t return_val = 0;
  if(imode == 0)  return_val = myhdedx0;
  if(imode == 1)  return_val = myhdedx1;
